...
class AppSignUpService < BaseService
  ...
  def create_user!
    ...
    @user.confirm! # verismile adicionado 2023/11/09
    @user.approve! # verismile adicionado 2023/11/09
  end
  ...
  def user_params
    @params.slice(:email, :password, :agreement, :locale, :time_zone, :full_name, :nationality, :birthday) # Verismile: adicionado:full_name, :nationality, :birthday
  end

  def account_params
    @params.slice(:username,:stripe_payment_intent) # Verismile: adicionado :stripe_payment_intent
  end
