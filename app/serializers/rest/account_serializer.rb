...
class REST::AccountSerializer < ActiveModel::Serializer
 ...
 # Verismile: adicionado full_name para retornar o valor na chamada /api/v1/account/verify_credentials
  attributes :id, :username, :acct, :display_name, :locked, :bot, :discoverable, :group, :created_at,
             :note, :url, :uri, :avatar, :avatar_static, :header, :header_static,
             :followers_count, :following_count, :statuses_count, :last_status_at, :full_name
  ...
  # Verismile
  def full_name
    user = User.find_by(account_id: object.id)
    user.full_name
  end
