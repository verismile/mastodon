...
class Api::V1::AccountsController < Api::BaseController
  ...
  #before_action :check_payment_confirmation, only: [:create] # Verismile: checar stripe_payment_intent
  before_action :check_secret, only: [:create] # Verismile
  ...
  # Removido 2023/11/08
  # Checar se o pagamento foi efetuado com sucesso, e obter os dados do usuário
  # def check_payment_confirmation
  #   Stripe.api_key = ENV.fetch('STRIPE_API_KEY')
  #   payment_intent = nil
  #   customer = nil
  #   begin
  #     payment_intent = Stripe::PaymentIntent.retrieve(params["stripe_payment_intent"])
  ...
  #     return
  #   end
  #   account_params[:email] = customer['email'] #obter o email do stripe, e não do POST
  #   account_params[:full_name] = customer['name']
  #   account_params[:birthday] = customer['metadata']['birthday']
  #   account_params[:nationality] = customer['metadata']['nationality']
  # end

  # proteger a rota /api/v1/accounts que cria conta, permitindo somente o middleware criar a conta
  def check_secret
    if (params["secret"] != ENV.fetch('SECRET'))
      forbidden
      return
    end
  end
  ...
  def account_params
    # Verismile: adicionado @account_params para que a variável seja editável em "create"
    @account_params ||= params.permit(:username, :email, :password, :agreement, :locale, :reason, :time_zone, :stripe_payment_intent, :full_name, :birthday, :nationality)
  end
